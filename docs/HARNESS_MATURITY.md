...
- Achieved. `score-trace`, enriched friction query, friction-by-component grouping,
  benchmark compare with responsibility attribution, and the backlog outcome loop
  are implemented. Run `bash scripts/verify-h3.sh` to verify.
...
- **Achieved.** `scripts/verify-story.sh` (single active task), `scripts/batch-verify.sh`
  (batch all stories + decisions), Stop hooks `run-harness-verify.mjs` + `sync-harness-story.mjs`
  + `backlog-surface.mjs`, `scripts/verify-h4.sh`, and [HARNESS_VERIFICATION.md](HARNESS_VERIFICATION.md)
  provide lane-aware lint/test, batch decision/story proof, and missing-evidence surfacing before
  a task is marked implemented. Generic multi-stack CI matrices remain future work.
