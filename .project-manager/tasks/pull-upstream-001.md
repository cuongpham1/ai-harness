...
**Files changed:** scripts/schema/003-tool-registry.sql, scripts/schema/004-intervention.sql, scripts/schema/005-tool-extensions.sql, scripts/bin/harness-cli (legacy baseline -> v0.1.10), docs/TOOL_REGISTRY.md, docs/HARNESS_AUDIT.md, docs/IMPROVEMENT_PROTOCOL.md, PHASE5.md, CHANGELOG.md, .gitattributes, docs/stories/US-019..025, docs/stories/US-027, CLAUDE.md (new, was missing at root)
**Errors:** Initial curl for scripts/bin/harness-cli returned 404 — the binary is not present in the repo tree (only crates/harness-cli source exists). Existing local binary was NOT clobbered because curl -f aborts before writing on 404.
